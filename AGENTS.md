...
This project uses Custom UI (not UI Kit). The frontend is a standard React app located in `static/ui/` and uses `@forge/bridge` to communicate with the resolver.
You MUST NOT import from `@forge/react` or `@forge/ui` in the Custom UI. Use standard React and `@forge/bridge` only.
The @forge/ui package is deprecated and MUST NOT be used.
...
This project uses Custom UI. When extending this app, continue using the Custom UI pattern in `static/ui/`.
...
The frontend is a standard React app in `static/ui/`. You may use standard HTML elements and any React-compatible libraries.
Use `invoke` from `@forge/bridge` to call resolver functions. Use `requestJira` from `@forge/bridge` for direct Jira API calls from the frontend without going through a resolver.
